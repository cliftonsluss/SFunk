...
template <typename num_t>
void variance01kd_r(string &filename, simFrame<num_t> &avg_frame, const size_t N,
    const int num_frames, const int num_nbs) {
  PointCloud<num_t> cloud;
  double skin = 2.5;
  size_t header = 5;
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    for (size_t k = 0; k < nbs; k++) {
      ...
      xdist_2 = pow(abs(xa-xb),2.0);
      ydist_2 = pow(abs(ya-yb),2.0);
      zdist_2 = pow(abs(za-zb),2.0);
...
//  string filename = "../../../lammps_EF/source/testing/run_04.dump";
